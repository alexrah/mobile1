...
class SimpleImageAleGalleryHelper {
	...
	public static function renderGallery($srcimgfolder, $thb_width, $thb_height, $smartResize, $jpg_quality, $cache_expire_time, $gal_id)
	{
		...
		// Internal parameters
		$prefix = "jw_sig_ale_cache_";

		// Set the cache folder
		$cacheFolderPath = JPATH_SITE.DS.'cache'.DS.'jw_sig_ale';
		...
		foreach ($found as $key => $filename)
		{
			...
			$gallery[$key]->thumbImageFilePath = $siteUrl.'cache/jw_sig_ale/'.$prefix.$gal_id.'_'.strtolower(self::cleanThumbName($thumbfilename));
