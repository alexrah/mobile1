...
class SimpleImageGalleryProMultiHelper
{
	...
	public static function renderGallery($srcimgfolder, $thb_width, $thb_height, $smartResize, $jpg_quality, $sortorder, $showcaptions, $itemTitle, $wordLimit, $cache_expire_time, $downloadFile, $gal_id, $resizeSrcImage)
	{
		...
		foreach ($found as $key => $filename)
		{
			...
			elseif ($showcaptions == 1)
			{
				...
				$gallery[$key]->captionDescription = JText::_('JW_SIGP_LABELS_04').'<br /><span class="sigpro_multiPopupCaption">'.$itemTitle."</span>";
			}
			else
			{
			...
			if ($downloadFile)
			{
				$gallery[$key]->downloadLink = self::replaceHtml('<br /><a class="sigpro_multiDownloadLink" href="'.$downloadFile.'?file='.$srcimgfolder.'/'.self::replaceWhiteSpace($filename).'">'.JText::_('JW_SIGP_LABELS_05').'</a>');
			}
			else
			{
			...
			// $gallery[$key]->thumbImageFilePath = $siteUrl.'cache/jw_sigpro_multi/'.$prefix.$gal_id.'_'.strtolower(self::cleanThumbName($thumbfilename));
