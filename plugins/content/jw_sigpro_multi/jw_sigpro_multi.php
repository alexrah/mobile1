<?php
/**
 * @version		$Id: jw_sigpro_multi.php 3445 2013-08-23 17:09:24Z joomlaworks $
...
class plgContentJw_sigpro_multi extends JPlugin
{

	// JoomlaWorks reference parameters
	var $plg_name = "jw_sigpro_multi";
	var $plg_tag = "gallery_multi";
	...
	function plgContentJw_sigpro_multi(&$subject, $params)
	{
	...
	function renderSIGP(&$row, &$params, $page = 0)
	{
		...
		$pluginParams = JComponentHelper::getParams('com_sigpro_multi');
		...
		$modulePosition = $loadmoduleposition ? htmlentities('<div class="sigProModulePosition">'.SimpleImageGalleryProMultiHelper::loadModulePosition($loadmoduleposition, -1).'</div>', ENT_QUOTES, 'utf-8') : NULL;
		...
		if (preg_match_all($regex, $row->text, $matches, PREG_PATTERN_ORDER) > 0)
		{
			...
			foreach ($matches[0] as $key => $match)
			{
				...
				if (strpos($tagcontent, 'www.flickr.com') === false)
				{
					...
					if(strpos($galleryFolder,'media/jw_sigpro_multi/users')!==false){
					...
					// Render the gallery
					$gallery = SimpleImageGalleryProMultiHelper::renderGallery($srcimgfolder, $gal_width, $gal_height, $smartResize, $jpg_quality, $sortorder, $gal_captions, $row->title, $wordLimit, $cache_expire_time, $downloadFile, $gal_id, $resizeSrcImage);
				...
				else
				{
					...
					if (preg_match_all($flickrRegex, $flickrSetUrl, $flickrMatches, PREG_PATTERN_ORDER) > 0)
					{
						...
						$getFlickrJson = SimpleImageGalleryProMultiHelper::readFile($flickrJson, 'jw_sigpro_multi');
						...
						foreach ($galleryData as $key => $photo)
						{
							...
							if ($wordLimit)
							{
								$gallery[$key]->captionTitle = SimpleImageGalleryProMultiHelper::wordLimit($gallery[$key]->captionTitle, $wordLimit);
							}
							...
							if ($downloadFile)
							{
								$gallery[$key]->downloadLink = SimpleImageGalleryProMultiHelper::replaceHtml('<br /><a class="sigProDownloadLink" target="_blank" href="'.$photo->url_o.'">'.JText::_('JW_SIGP_LABELS_13').'</a>');
							}
							else
							{
				...
				if (JRequest::getCmd('format') == '' || JRequest::getCmd('format') == 'html')
				{
					...
					$pluginCSS = SimpleImageGalleryProMultiHelper::getTemplatePath($this->plg_name, 'css/template.css', $gal_template);
				...
				// Fetch the template
				ob_start();
				$templatePath = SimpleImageGalleryProMultiHelper::getTemplatePath($this->plg_name, 'default.php', $gal_template);
